---
layout: post
title: Confs e Empregos
date: 2025-07-20 2025-07-20 09:30:00+0200
description: Neste post, listo alguns links que costumo acompanhar para ficar por dentro de conferências e oportunidades de emprego nas áreas de relatividade e física quântica em geral.
tags: General-Relativity Quantum-Mechanics RQI QFT
categories: useful-links conferences open-positions
featured: true
related_posts: false
---

### Links úteis para conferências e oportunidades de emprego

Abaixo está uma lista de sites que costumo acompanhar para me manter atualizado sobre conferências e vagas acadêmicas ou de pesquisa:

- [`ISRQI`](https://isrqi.net/#home), International Society for Relativistic Quantum Information. Organiza eventos e compartilha atualizações relacionadas à teoria da informação quântica relativística.

- [`QT-lab`](https://quantum.technology/conf/index.html), Lista de Conferências em Tecnologia Quântica. Mantém um calendário de conferências nas áreas de informação quântica, óptica quântica e campos relacionados.

- [`Quantiki`](https://www.quantiki.org/), Um centro para a comunidade de ciência da informação quântica. Oferece listas de empregos, anúncios de conferências e diretórios de grupos de pesquisa.

- [`Hyperspace`](https://hyperspace.uni-frankfurt.de/), Um recurso central para anúncios nas áreas de relatividade geral e gravitação, incluindo conferências, workshops e oportunidades de emprego.

